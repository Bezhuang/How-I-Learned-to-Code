 ...
 * match the following terms. So much so, that they would like a daily report of
 * the number of products for each term along with the total product count.
 */
@RestController
public class ReportController {

    // After reading code/tests, we can capture the important terms in an array!
    private static final String[] importantTerms = new String[] { "Cool", "Amazing", "Perfect", "Kids" };
    ...
    public SearchReportResponse runReport() {
        // We could use the search service and do an empty string query to get the count
        // but this is much more efficient
        Number count = (Number) this.entityManager.createQuery("SELECT count(item) FROM ProductItem item")
                .getSingleResult();
